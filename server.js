...
let namespaces = require('./src/namespaces');
...
namespaces.forEach((namespace) => {
  ...
  io.of(namespace.endpoint).on('connection', (nsSocket) => {
    ...
    nsSocket.on('joinRoom', (roomToJoin, numberOfUsersCallback) => {
      ...
      const roomsToLeave = Object.keys(nsSocket.rooms);
    ...
    nsSocket.on('newMessageToServer', (msg) => {
      ...
      // get the keys
      const roomTitle = Object.keys(nsSocket.rooms)[0];
...
function updateUsersInRoom(namespace, room) {
  console.log(namespace.endpoint, room);
